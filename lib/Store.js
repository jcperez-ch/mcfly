 ...
 */
var _recursivelyMergeState = function _recursivelyMergeState(initialState, namespace, state) {
  var multilevelNamespace = namespace.match(/^[a-z\d]*\./);

  if (multilevelNamespace !== null) {
    var parentNamespace = namespace.substring(0, multilevelNamespace[0].length - 1);
    var childNamespace = namespace.substring(multilevelNamespace[0].length);
    return Object.freeze((0, _lodash.assign)({}, initialState, _defineProperty({}, parentNamespace, _recursivelyMergeState((0, _lodash.get)(initialState, parentNamespace), childNamespace, state))));
  }

  return Object.freeze((0, _lodash.assign)({}, initialState, _defineProperty({}, namespace, state)));
};
...
var Store = function () {
  ...
  _createClass(Store, [{
    ...
    key: 'mergeState',
    value: function mergeState(namespace, state) {
      var prevState = (0, _lodash.get)(this.state, namespace);
      if (prevState !== state) {
        var newState = Object.isFrozen(state) ? state : Object.freeze(state);
        this.state = _recursivelyMergeState(this.state, namespace, newState);
      }
    }
  }, {
    key: 'emitChangeIfStoreChanged',
    value: function emitChangeIfStoreChanged(newState) {
...
var MasterStore = function (_Store) {
  ...
  function MasterStore() {
    ...
    var _this2 = _possibleConstructorReturn(this, Object.getPrototypeOf(MasterStore).call(this, {}, undefined, initialState));
  ...
  _createClass(MasterStore, [{
    key: 'getDispatchTokens',
    value: function getDispatchTokens() {
